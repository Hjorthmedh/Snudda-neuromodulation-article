...
# ── Helper ───────────────────────────────────────────────────────────────────
def polish_ax(ax, x_data, xlabel, ylabel):
    """Apply publication styling after data has been plotted."""
    # Explicitly lock x-axis to data range + 6% padding on each side.
    # Without this, axvline(0) or AutoMinorLocator can silently expand the
    # axis beyond the rightmost data point, making the line look clipped.
    x_pad = (x_data.max() - x_data.min()) * 0.06
    ax.set_xlim(x_data.min() - x_pad, x_data.max() + x_pad)

    # Give y a little breathing room too so top/bottom markers aren't clipped.
    ylo, yhi = ax.get_ylim()
    y_pad = (yhi - ylo) * 0.08
    ...
    # Δt = 0 reference line — only if 0 is inside the data range
