...
for idx, nid in enumerate(neuron_id):
    ...
    try:
        ...
        area.append(np.sum((conc - np.min(conc))[:-1] * np.diff(time[t_idx])) * MM_TO_UM)
    except Exception:
        import traceback
...
polish_ax(ax2, x,
          xlabel=r"$\Delta t$ (s)",
          ylabel=r"Area substrate ($\mu$M$\cdot$s)")
fig2.tight_layout()
