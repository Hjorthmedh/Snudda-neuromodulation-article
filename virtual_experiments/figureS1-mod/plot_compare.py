"""Faceted RxD vs .mod comparison, one species per panel, plotly.
...
cols = 3
rows = (n + cols - 1) // cols
fig = make_subplots(rows=rows, cols=cols, subplot_titles=SPECIES,
                    horizontal_spacing=0.06, vertical_spacing=0.08)

for i, s in enumerate(SPECIES):
    r, c = i // cols + 1, i % cols + 1
    if rxd.get(s) is not None:
        fig.add_trace(go.Scatter(x=t_rxd, y=rxd[s], mode="lines", name="RxD",
                                 line=dict(color="#1f77b4", width=2),
                                 showlegend=(i == 0)),
                      row=r, col=c)
    if mod.get(s) is not None:
        fig.add_trace(go.Scatter(x=t_mod, y=mod[s], mode="lines", name="mod (Nair)",
