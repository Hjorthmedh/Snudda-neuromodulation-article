...
COMMENT
	...
	Pointers MUST be set from Python after instantiation:
	  ...
	  h.setpointer(rxd_da_node._ref_concentration,  "da_ref", seg.Nair_2016_optimized_ptr)
	If a pointer is unset, reading it crashes NEURON.
ENDCOMMENT

NEURON {
	SUFFIX Nair_2016_optimized_ptr : OR perhaps POINT_PROCESS ?
	RANGE O1 : output
...
ASSIGNED {
	: POINTER-read inputs. Set from Python via h.setpointer before run.
	ca_ref (millimole/liter)
	da_ref (millimole/liter)
	: per-step net cal consumption rate, used for write-back
...
PROCEDURE assign_calculated_values() {
	...
	Ca = ca_ref : cytosolic Ca read directly from RxD cal node (mM)
	DA = da_ref : DA read directly from RxD DA node (mM)
...
BREAKPOINT {
	...
	: cal is rxd.Species, so RxD reactions in the full-cascade run consume it.
	: This .mod can't participate in RxD's reaction graph, so we mirror the
	: consumption by writing it back through ca_ref. Stoichiometry matches the
	: SBtab spec: 1 cal per Ca-binding reaction (regardless of how many Ca
	: atoms the product name implies). Forward-Euler over dt.
	ca_rate = -(R5 + R16 + R24 + R25 + R26 + R28 + R54 + R73 + R74 + R75 + R76 + R115 + R116 + R123 + R124)
	ca_ref  = ca_ref + ca_rate * dt
	: DA stays read-only - DA is still rxd.Parameter (bath-clamped), so the
	: rxd reference also doesn't consume it. Adding a da_ref write would
	: introduce a sink that the reference doesn't have.
}
DERIVATIVE ode {
